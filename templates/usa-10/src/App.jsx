...
function App() {
  ...
  function handleDiagnose() {
    const sym = inputRef.current?.value?.trim() || 'AAPL'
    setDiagSymbol(sym)
    setDiagnosing(true)
    setDiagnoseStep(0)
    setDiagProgress(0)
    ...
    const duration = 2000
    const stepDurations = [400, 500, 500, 600] // ms each step
    ...
    const tick = () => {
      ...
      for (let i = 0; i < DIAG_STEPS.length; i++) {
        elapsedForStep += stepDurations[i]
        if (elapsed >= elapsedForStep && !stepDone.current[i]) {
          stepDone.current[i] = true
          setDiagnoseStep(i + 1)
        }
      }
  ...
  return (
    <div className="app">
      ...
      {diagnosing && (
        <div className="diagnose-overlay">
          <div className="diagnose-card">
            ...
            <p className="diagnose-title">Diagnosing <strong>{diagSymbol}</strong></p>
            <div className="diagnose-steps">
              {DIAG_STEPS.map((step, i) => (
                <div key={i} className={'diag-step' + (i < diagnoseStep ? ' done' : '') + (i === diagnoseStep ? ' active' : '')}>
                  <span className="diag-step-icon">{i < diagnoseStep ? '✓' : i === diagnoseStep ? '●' : '○'}</span>
                  <span className="diag-step-label">{step}</span>
                  {i < diagnoseStep && <span className="diag-step-ok">OK</span>}
                </div>
              ))}
            </div>
        ...
        <div className="testimonial-card">
          ...
          <div className="user">
            ...
            <div>
              ...
              <div className="user-role">Data Analyst &middot; Texas</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer className="js-reveal revealed">
        <p className="copy">&copy; 2026 AI Stock Diagnosis Platform</p>
      ...
      <dialog className="modal" id="modalPrivacy">
        <div className="modal-head"><div className="modal-title">Privacy Policy</div><button className="modal-close" onClick={() => document.getElementById('modalPrivacy')?.close()}>Close &times;</button></div>
        <div className="modal-body">
          <p style={{color:'#6b7280',fontSize:13,marginBottom:16}}>Last updated: May 2026</p>
          <h2>1. Who We Are</h2><p>This website and the AI Stock Diagnosis service are operated by AI Stock Diagnosis Platform.</p>
          <h2>2. What Information We Collect</h2><h3>a) Information you provide directly</h3><ul><li><strong>Contact details</strong>, such as your name, email or phone number.</li><li><strong>Messages and content</strong> you send to us via the platform.</li></ul><h3>b) Information collected automatically</h3><p>When you visit this website, we may receive basic technical information including IP address, timestamp, pages viewed, and device information.</p>
          <h2>3. How We Use Your Information</h2><ul><li><strong>Provide the Service</strong> — to deliver AI-powered stock analysis.</li><li><strong>Operate and secure the website</strong> — to monitor, maintain and improve the site.</li><li><strong>Comply with legal obligations</strong>.</li></ul>
          <h2>4. How We Share Information</h2><p>We do <strong>not</strong> sell your personal information. We may share limited information with service providers under confidentiality obligations.</p>
          <h2>5. Your Rights</h2><p>Depending on your location, you may have rights such as access, correction, or deletion of your personal information.</p>
          <h2>6. Security</h2><p>We take reasonable measures to protect personal information.</p>
          <h2>7. Changes</h2><p>We may update this Privacy Policy. Continued use constitutes acceptance.</p>
        </div>
      </dialog>

      <dialog className="modal" id="modalDisclaimer">
        <div className="modal-head"><div className="modal-title">Terms of Service</div><button className="modal-close" onClick={() => document.getElementById('modalDisclaimer')?.close()}>Close &times;</button></div>
        <div className="modal-body">
          <p style={{color:'#6b7280',fontSize:13,marginBottom:16}}>Last updated: May 2026</p>
          <h2>1. Educational Purposes Only</h2><p>All content is for <strong>general informational and educational purposes only</strong>. Nothing on this website is personalized financial, investment or trading advice.</p>
          <h2>2. No Guarantees</h2><p><strong>Past performance is not a reliable indicator of future results.</strong> We make no performance, profit or outcome guarantees.</p>
          <h2>3. Independent Decisions</h2><p>Before making any investment decision, conduct your own independent research and consider consulting a qualified professional.</p>
          <h2>4. No Client Relationship</h2><p>The use of this website does <strong>not</strong> create any advisory, fiduciary, or client relationship.</p>
          <h2>5. Data Sources</h2><p>Content may be based on data from public sources. We cannot guarantee accuracy or completeness.</p>
          <h2>6. No Responsibility for Losses</h2><p>We shall not be liable for any damages or losses arising from your use of the Content.</p>
          <h2>7. Changes</h2><p>We may update these Terms from time to time.</p>
        </div>
      </dialog>
    </div>
  )
